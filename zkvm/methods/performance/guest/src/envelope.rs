//! SourceEnvelope v1 canonical digest, mirroring
//! `services/collector/attestation::envelope::SourceEnvelope` field-for-
//! field (both are independent implementations of the same
//! the protocol specification "SourceEnvelope v1" canonical encoding —
//! `services/` and `zkvm/` are separate Cargo workspaces, so the type
//! cannot be shared directly). The guest recomputes this digest itself
//! from the witnessed field values rather than trusting a host-supplied
//! digest, so a host cannot sign one envelope and feed the guest a
//! different one under the same claimed digest.
